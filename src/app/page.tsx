    ...
    <>
      ...
      <div className="absolute left-0 right-0 bottom-0 z-50 transition-transform duration-500"
        ...
        <div className="bg-[#1a1a1a] rounded-t-[28px] px-5 pb-10 pt-3">
          ...
          {/* Wording fix #2: "Autorizar débito" */}
          ...
          {/* Slider — matches real Lemon screenshot exactly:
              - Pill shape (very rounded, like a capsule)
              - Cream/off-white background (#f5f0e8)
              - Black circular thumb on the left with chevron
              - Dark text "Desliza para confirmar" centered
              - No green fill — just the thumb moves
              - Border with subtle stroke */}
          <div className="relative h-[58px] rounded-full overflow-hidden select-none touch-none border border-[#d5d0c8]/30"
            style={{ background: "#f5f0e8" }}>

            {/* Centered text */}
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none"
              style={{ opacity: Math.max(0, 1 - swipeX / 0.3) }}>
              <span className="text-[#3a3a3a] text-[16px] font-medium tracking-lemon pl-8">
...
function LockScreen({ showNotif, onTapNotif, pull }: { showNotif: boolean; onTapNotif: () => void; pull: Pull }) {
  ...
  return (
    <div className="h-full relative flex flex-col items-center" style={{ background: "linear-gradient(170deg, #1a1a2e 0%, #111122 30%, #0d0d1a 60%, #000 100%)" }}>
      ...
      {/* Wording fix #1: Push more direct, no emoji */}
      {showNotif && (
...
function SuccessScreen({ pull }: { pull: Pull }) {
  ...
  return (
    <div className="h-full bg-black flex flex-col items-center justify-center fade-in relative overflow-hidden">
      ...
      <p className="text-t-secondary text-[14px] text-center px-8 mb-6 tracking-lemon">
        Se debitaron ${fmtARS(pull.amount)} de tu cuenta en {pull.bankName}
      </p>
...
/* ================================================================
   ACTIVITY — NOTIFICACIONES (fix #7: no "Rechazada por el usuario" badge)
   ================================================================ */
