...
type NotifType = "pending" | "success" | "rejected" | "expired" | "insufficient";

interface Pull { bankName: string; amount: number; concept: string; cbu: string; recipientName: string; currentBalance: number; }
interface Notif { id: string; type: NotifType; pull: Pull; time: string; }

const PULL_1: Pull = { bankName: "Brubank", amount: 500000, concept: "Fondeo cuenta propia", cbu: "1430001713001150420010", recipientName: "Jeronimo Campero", currentBalance: 265215.22 };
const PULL_2: Pull = { bankName: "Banco Galicia", amount: 120000, concept: "Fondeo cuenta propia", cbu: "0070999020000008210374", recipientName: "Jeronimo Campero", currentBalance: 765215.22 };

function now() { return new Date().toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit", hour12: false }); }
let nid = 0;
function mkNotif(type: NotifType, pull: Pull): Notif { return { id: `n-${++nid}`, type, pull, time: now() }; }
...
export default function Page() {
  ...
  // Notification log — chronological, newest first when displayed
  const [notifs, setNotifs] = useState<Notif[]>([]);
  const [tappedNotif, setTappedNotif] = useState<Notif | null>(null);

  const addNotif = useCallback((type: NotifType, pull: Pull) => {
  ...
  const startFlow = useCallback((m: SimMode) => {
    ...
    if (m === "pending") {
      // Go straight to notificaciones with a pending notif
      addNotif("pending", PULL_1);
      setScreen("activity_notif");
      return;
    }

    setScreen("lockscreen"); setShowPush(false); setShowSheet(false);
    if (m === "double") setQueuedPull(PULL_2);

    // Push arrives → add pending notif
    setTimeout(() => {
      setShowPush(true);
      addNotif("pending", PULL_1);
      if (m === "double") {
        setTimeout(() => addNotif("pending", PULL_2), 700);
      }
    }, 1500);
  }, [addNotif]);

  const tapPush = useCallback(() => {
    setShowPush(false);
    setScreen("biometric");
    setTimeout(() => {
      if (mode === "expired") {
  ...
  const handleConfirm = useCallback(() => {
    ...
    setTimeout(() => {
      if (queuedPull) {
        ...
        setScreen("modal");
        setTimeout(() => setShowSheet(true), 400);
      } else { setScreen("activity_mov"); }
    }, 2500);
  }, [mode, activePull, queuedPull, addNotif]);
  ...
  const handleReject = useCallback(() => {
    ...
    if (queuedPull) {
      setActivePull(queuedPull); setQueuedPull(null);
      setTimeout(() => { setScreen("modal"); setTimeout(() => setShowSheet(true), 400); }, 300);
    } else { setScreen("activity_notif"); }
  }, [activePull, queuedPull, addNotif]);
  ...
  const handleNotifTap = useCallback((n: Notif) => {
    if (n.type === "pending") {
      setActivePull(n.pull); setMode("pending");
      setScreen("modal"); setTimeout(() => setShowSheet(true), 300);
    } else {
      setTappedNotif(n);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-neutral-950 p-4 gap-4">
      ...
      <div className="w-[393px] h-[852px] bg-black rounded-[55px] border-[3px] border-[#333] overflow-hidden relative shadow-2xl shadow-black/60">
        ...
        {screen === "activity_notif" && (
          <ActivityNotifScreen
            ...
            tappedNotif={tappedNotif}
            onCloseSheet={() => setTappedNotif(null)}
          />
        )}
      ...
      {showRestart && (
        <button onClick={restart} className="flex items-center gap-2 px-6 py-3 rounded-2xl border border-[#444] text-[#999] text-[14px] font-medium tracking-lemon active:scale-[0.97] transition-transform">
...
function SimBtn({ label, green, onClick }: { label: string; green?: boolean; onClick: () => void }) {
  return <button onClick={onClick} className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl font-medium text-[13px] tracking-lemon active:scale-[0.97] transition-transform ${green ? "" : "border border-[#444] text-[#999]"}`} style={green ? { background: "#00f068", color: "#000" } : {}}>{green && <BellIcon color="black" />}{label}</button>;
}
  ...
  return (
    <div className="h-full bg-black flex flex-col">
      ...
      <div className="px-5 mb-5">
        ...
        <div className="flex items-baseline gap-2"><span className="text-white text-[42px] font-bold tracking-tight leading-none">{fmtParts(pull.currentBalance / 1476).int}<span className="text-t-secondary text-[24px]">,{fmtParts(pull.currentBalance / 1476).dec}</span></span><span className="text-t-secondary text-[18px] font-medium">USDT</span></div>
        <p className="text-t-tertiary text-[14px] tracking-lemon mt-0.5">{fmtARS(pull.currentBalance)} ARS</p>
      </div>
      <div className="flex gap-4 px-5 mb-6"><ActionBtn icon={<ArrowDown />} label="DEPOSITAR" active /><ActionBtn icon={<RefreshSvg />} label="CAMBIAR" /><ActionBtn icon={<ArrowUp />} label="ENVIAR" /></div>
      <div className="mx-5 bg-[#1a1a1a] rounded-2xl p-4 mb-6 border border-[#222]"><p className="text-white text-[15px] font-medium tracking-lemon mb-1">Transferencias Pull 🔄</p><p className="text-t-secondary text-[13px] leading-relaxed tracking-lemon">Ahora podés autorizar débitos desde tus cuentas en otros bancos directamente en Lemon.</p></div>
      <div className="px-5 mb-4">
        <div className="flex justify-between items-center mb-1"><span className="text-t-secondary text-[14px] tracking-lemon">Pesos</span><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#666" strokeWidth="2" strokeLinecap="round"><path d="M9 18l6-6-6-6"/></svg></div>
        <div className="flex items-baseline gap-2"><span className="text-white text-[26px] font-bold tracking-tight">{fmtParts(pull.currentBalance / 1476).int}</span><span className="text-[16px] font-medium" style={{ color: "#00f068" }}>USDT</span><div className="ml-auto px-2.5 py-1 rounded-full" style={{ background: "rgba(0,240,104,0.15)" }}><span className="text-[12px] font-medium" style={{ color: "#00f068" }}>19.97%</span></div></div>
        <p className="text-t-tertiary text-[13px] tracking-lemon">{fmtARS(pull.currentBalance)} ARS</p>
      </div>
      <div className="flex-1" /><TabBar />
    </div>
  );
}
...
function SheetOverlay({ show, pull, onConfirm, onReject }: { show: boolean; pull: Pull; onConfirm: () => void; onReject: () => void }) {
  const [swipeX, setSwipeX] = useState(0); const [swiping, setSwiping] = useState(false); const [startX, setStartX] = useState(0);
  const a = fmtParts(pull.amount);
  useEffect(() => { setSwipeX(0); setSwiping(false); }, [pull]);
  const dn = (e: React.PointerEvent) => { (e.target as HTMLElement).setPointerCapture(e.pointerId); setSwiping(true); setStartX(e.clientX); };
  const mv = (e: React.PointerEvent) => { if (!swiping) return; setSwipeX(Math.max(0, Math.min(1, (e.clientX - startX) / 260))); };
  const up = () => { setSwiping(false); if (swipeX > 0.7) { setSwipeX(1); setTimeout(onConfirm, 250); } else setSwipeX(0); };
  return (
    <>
      ...
      <div className="absolute left-0 right-0 bottom-0 z-50 transition-transform duration-500" style={{ transform: show ? "translateY(0)" : "translateY(100%)", transitionTimingFunction: "cubic-bezier(0.16,1,0.3,1)" }}>
        <div className="bg-[#1a1a1a] rounded-t-[28px] px-5 pb-10 pt-3">
          ...
          <div className="flex items-baseline justify-center gap-3 mb-5"><div className="self-center"><RoundFlag size={36} /></div><span className="text-white text-[32px] font-bold tracking-tight leading-none">{a.int}</span><span className="text-t-secondary text-[20px] font-medium leading-none">ARS</span></div>
          <div className="h-px bg-[#333] mb-4" />
          <div className="space-y-3.5 mb-4"><DetailRow label="Nombre" value={pull.recipientName} /><DetailRow label="Banco" value={pull.bankName} /><DetailRow label="CBU" value={pull.cbu} mono /></div>
          ...
          <div className="relative h-[58px] rounded-full overflow-hidden select-none touch-none border border-[#d5d0c8]/30" style={{ background: "#f5f0e8" }}>
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ opacity: Math.max(0, 1 - swipeX / 0.3) }}><span className="text-[#555] text-[15px] font-medium tracking-lemon pl-8">Desliza para confirmar</span></div>
...
function ExpiredSheet({ pull, show, onBack }: { pull: Pull; show: boolean; onBack: () => void }) {
  return <InfoSheet show={show} onBack={onBack} icon={<ClockIcon />} iconBg="rgba(239,159,39,0.12)" title="Esta solicitud expiró" body={<>La solicitud de débito de <W>{pull.bankName}</W> por <W>${fmtARS(pull.amount)} ARS</W> ya no está disponible.</>} sub="No se debitó nada de tu cuenta." btn="Volver al inicio" />;
}
  ...
  return (
    <>
      ...
      <div className="absolute left-0 right-0 bottom-0 z-50 transition-transform duration-500" style={{ transform: show ? "translateY(0)" : "translateY(100%)", transitionTimingFunction: "cubic-bezier(0.16,1,0.3,1)" }}>
        <div className="bg-[#1a1a1a] rounded-t-[28px] px-5 pb-10 pt-3">
          <div className="flex justify-center mb-5"><div className="w-10 h-1 rounded-full bg-[#444]" /></div>
          <div className="flex justify-center mb-4"><div className="w-[60px] h-[60px] rounded-full flex items-center justify-center" style={{ background: "rgba(0,240,104,0.12)" }}><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#00f068" strokeWidth="2.5" strokeLinecap="round"><path d="M5 12l5 5L20 7"/></svg></div></div>
          <p className="text-white text-[18px] font-bold tracking-lemon mb-2 text-center">Débito autorizado</p>
          <p className="text-t-secondary text-[14px] text-center leading-relaxed tracking-lemon mb-4">Se debitaron <W>${fmtARS(pull.amount)} ARS</W> de tu cuenta en <W>{pull.bankName}</W></p>
          <div className="h-px bg-[#333] mb-4" />
          <div className="space-y-3 mb-4"><DetailRow label="Nombre" value={pull.recipientName} /><DetailRow label="Banco" value={pull.bankName} /><DetailRow label="CBU" value={pull.cbu} mono /><DetailRow label="Nuevo saldo" value={`$${fmtARS(nb)} ARS`} green /></div>
...
function RejectedDetailSheet({ pull, show, onBack }: { pull: Pull; show: boolean; onBack: () => void }) {
  return <InfoSheet show={show} onBack={onBack} icon={<XCircleIcon />} iconBg="rgba(226,75,74,0.12)" title="Transferencia Pull rechazada" body={<>Rechazaste la solicitud de débito de <W>{pull.bankName}</W> por <W>${fmtARS(pull.amount)} ARS</W>.</>} sub="No se debitó nada de tu cuenta." btn="Cerrar" />;
}
function W({ children }: { children: React.ReactNode }) { return <span className="text-white font-medium">{children}</span>; }

/* ================================================================ LOCK SCREEN ================================================================ */
function LockScreen({ showPush, onTapPush, pull, queuedPull }: { showPush: boolean; onTapPush: () => void; pull: Pull; queuedPull: Pull | null }) {
  const [time, setTime] = useState(""); const [date, setDate] = useState(""); const [show2, setShow2] = useState(false);
  useEffect(() => { const t = () => { const d = new Date(); setTime(d.toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit", hour12: false })); setDate(d.toLocaleDateString("en-US", { weekday: "short", day: "numeric", month: "short" })); }; t(); const i = setInterval(t, 1000); return () => clearInterval(i); }, []);
  useEffect(() => { if (showPush && queuedPull) { const t = setTimeout(() => setShow2(true), 700); return () => clearTimeout(t); } setShow2(false); }, [showPush, queuedPull]);
  return (
    <div className="h-full relative flex flex-col items-center" style={{ background: "linear-gradient(170deg, #1a1a2e 0%, #111122 30%, #0d0d1a 60%, #000 100%)" }}>
      ...
      {showPush && queuedPull && show2 && <div className="absolute top-[230px] left-4 right-4 notif-enter cursor-pointer" onClick={onTapPush}><PushNotif pull={queuedPull} /></div>}
      {showPush && <div className={`absolute left-4 right-4 notif-enter cursor-pointer ${queuedPull && show2 ? "top-[370px]" : "top-[230px]"}`} style={{ transition: "top 0.4s cubic-bezier(0.16,1,0.3,1)" }} onClick={onTapPush}><PushNotif pull={pull} /></div>}
      <div className="absolute bottom-10 left-0 right-0 flex justify-between px-12"><div className="w-[50px] h-[50px] rounded-full bg-white/[0.06] flex items-center justify-center"><svg width="20" height="22" viewBox="0 0 20 22" fill="white" opacity="0.4"><path d="M10 1L2 6v10l8 5 8-5V6l-8-5z"/></svg></div><div className="w-[50px] h-[50px] rounded-full bg-white/[0.06] flex items-center justify-center"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.5" opacity="0.4"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg></div></div>
      {showPush && <p className="absolute bottom-[88px] text-white/25 text-[12px] tracking-lemon">Tocá la notificación</p>}
    </div>
  );
}
...
function SuccessScreen({ pull }: { pull: Pull }) {
  ...
  return (
    <div className="h-full bg-black flex flex-col items-center justify-center fade-in relative overflow-hidden">
      ...
      <div className="bg-[#1a1a1a] rounded-2xl px-6 py-4 text-center slide-up" style={{ animationDelay: "0.15s" }}><p className="text-t-tertiary text-[11px] uppercase tracking-widest mb-1">Tu nuevo saldo en Lemon</p><p className="text-[24px] font-bold" style={{ color: "#00f068" }}>${fmtARS(nb)} ARS</p></div>
    </div>
  );
}
...
/* ================================================================ ACTIVITY — NOTIFICACIONES (chronological log) ================================================================ */
const NCFG: Record<NotifType, { icon: React.ReactNode; iconBg: string; title: string; color: string }> = {
...
const NBODY: Record<NotifType, (p: Pull) => React.ReactNode> = {
  ...
  insufficient: p => <>No había saldo suficiente en tu cuenta Lemon para el débito de <span className="text-white">${fmtARS(p.amount)} ARS</span> solicitado por <span className="text-white">{p.bankName}</span>.</>,
};

function ActivityNotifScreen({ notifs, onMov, onTapNotif, tappedNotif, onCloseSheet }: {
  notifs: Notif[]; onMov: () => void; onTapNotif: (n: Notif) => void; tappedNotif: Notif | null; onCloseSheet: () => void;
}) {
  return (
    <div className="h-full bg-black flex flex-col relative">
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-3">
        {notifs.length === 0 ? (
          ...
          return (
            <div key={n.id} className="bg-[#1a1a1a] rounded-2xl p-4 border border-[#2a2a2a] slide-up cursor-pointer active:bg-[#222] transition-colors" onClick={() => onTapNotif(n)}>
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: cfg.iconBg }}>{cfg.icon}</div>
                <div className="flex-1">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-white font-medium text-[14px] tracking-lemon">{cfg.title}</span>
                    <span className="text-t-tertiary text-[12px]">{n.time}</span>
                  </div>
                  <p className="text-t-secondary text-[13px] leading-snug tracking-lemon">{NBODY[n.type](n.pull)}</p>
                  {n.type === "pending" && (
                ...
                <div className="flex items-center justify-center mt-1 flex-shrink-0"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#666" strokeWidth="2" strokeLinecap="round"><path d="M9 18l6-6-6-6"/></svg></div>
              </div>
            </div>
          );
        })}
      ...
      {/* Sheets triggered by tapping a notif card */}
      {tappedNotif?.type === "success" && <SuccessDetailSheet pull={tappedNotif.pull} show onBack={onCloseSheet} />}
      {tappedNotif?.type === "rejected" && <RejectedDetailSheet pull={tappedNotif.pull} show onBack={onCloseSheet} />}
      {tappedNotif?.type === "expired" && <ExpiredSheet pull={tappedNotif.pull} show onBack={onCloseSheet} />}
      {tappedNotif?.type === "insufficient" && <InsufficientSheet pull={tappedNotif.pull} show onBack={onCloseSheet} />}
    </div>
  );
}
